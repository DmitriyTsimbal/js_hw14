...
//(весь текст масиву, що в кінці документа скопіюєте в код).
...
document.write(`Items total price: ${ getTotalPrice(dataList) }₴<hr>`)


// 2)Знайти кількість товарів, у яких ціна зменшилась (price < old_price).

const getLoweredPrice = (dataList) => {
    let itemsCounter = 0;
    for (let data of dataList) {
        if (data.price < data.old_price) itemsCounter++
    }
    return itemsCounter;
}

document.write(`The number of products whose price has decreased: ${ getLoweredPrice(dataList) } <hr>`)

// 3) Товари, які доступні (sell_status:"available")
const getAvailableItems = (dataList) => {
    let availableItemsCounter = 0;
    for (let data of dataList) {
        if (data.sell_status === "available") availableItemsCounter++
    }
    return availableItemsCounter
}

document.write(`Available items count: ${ getAvailableItems(dataList) } <hr>`)

// 4) сформувати новий список об'єктів тільки доступних для продажу товарів, які міститимуть тільки ідентифікатор товару (id), нову ціну (price), стару ціну (old_price), та ціну у доларах (usd_price)

const getAvailableItemsList = (dataList) => {
    const resultArray = [];
    for (let data of dataList) {
        if (data.sell_status === "available") {
            resultArray.push({
                id: data.id,
                price: data.price,
...
const renderAvailableItems = (availableItems) => {
    for (let items of availableItems) {
        document.write(`
        Item id: ${ items.id }<br>
        Item price: ${ items.price }₴<br>
        Item old price: ${ items.old_price }₴<br>
        Price in dollars: ${ items.usd_price }$<hr>
        `)
    }
}

const AVAILABLE_ITEMS = getAvailableItemsList(dataList)
renderAvailableItems(AVAILABLE_ITEMS)